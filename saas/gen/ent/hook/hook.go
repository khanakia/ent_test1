...
// The MediableFunc type is an adapter to allow the use of ordinary
// function as Mediable mutator.
type MediableFunc func(context.Context, *ent.MediableMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MediableFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MediableMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MediableMutation", m)
}
