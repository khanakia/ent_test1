...
func (pcq *PostCategoryQuery) FirstIDX(ctx context.Context) string {
...
func (pcq *PostCategoryQuery) OnlyIDX(ctx context.Context) string {
...
func (pcq *PostCategoryQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (pcq *PostCategoryQuery) IDsX(ctx context.Context) []string {
	ids, err := pcq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pcq *PostCategoryQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*PostCategory, init func(*PostCategory), assign func(*PostCategory, *Post)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(post.FieldPrimaryCategoryID)
	}
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(postcategory.PostsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.PrimaryCategoryID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "primary_category_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pcq *PostCategoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(postcategory.Table, postcategory.Columns, sqlgraph.NewFieldSpec(postcategory.FieldID, field.TypeString))
	_spec.From = pcq.sql
...
func (pcq *PostCategoryQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *PostCategoryQuery {
	query := (&PostClient{config: pcq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pcq.withNamedPosts == nil {
