...
// The MediableFunc type is an adapter to allow the use of ordinary function as a Querier.
type MediableFunc func(context.Context, *ent.MediableQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f MediableFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.MediableQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.MediableQuery", q)
}

// The TraverseMediable type is an adapter to allow the use of ordinary function as Traverser.
type TraverseMediable func(context.Context, *ent.MediableQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseMediable) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseMediable) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.MediableQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.MediableQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.MediableQuery:
		return &query[*ent.MediableQuery, predicate.Mediable, mediable.OrderOption]{typ: ent.TypeMediable, tq: q}, nil
	case *ent.PostQuery:
